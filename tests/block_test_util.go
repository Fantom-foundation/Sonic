...
func (t *BlockTest) Run(factory testStateDBFactory) error {
	_, ok := tests.Forks[t.Data().Network]
	if !ok {
		return tests.UnsupportedForkError{t.Data().Network}
	}

	// init genesis state
	db := factory.newTestStateDB(t.Data().Pre)

	if got, want := db.GetStateHash(), t.Data().Genesis.StateRoot; got != want {
		return fmt.Errorf("genesis state root does not match: got: %x != want: %x", got, want)
	}

	//evmm := evmmodule.New()
	//block
	//evmm.Start(nil, db, nil, nil, nil, nil)
	//
	//// Wrap the original engine within the beacon-engine
	//engine := beacon.New(ethash.NewFaker())
	//
	//chain, err := core.NewBlockChain(db, cache, gspec, nil, engine, vm.Config{ // TODO equivalent v Oepře???
	//	Tracer:                  tracer,
	...
	//defer chain.Stop()
	//
	//validBlocks, err := t.insertBlocks(chain) // TODO tady se naplni blockchain bloky podle Blocks// TODO equivalent v Oepře???
	//if err != nil {
	//	return err
	//}
	//
	//cmlast := chain.CurrentBlock().Hash()
	//if common.Hash(t.Data().BestBlock) != cmlast {
	//	return fmt.Errorf("last block hash validation mismatch: want: %x, have: %x", t.Data().BestBlock, cmlast)
	//}
