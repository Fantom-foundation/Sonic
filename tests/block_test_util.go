...
func (t *BlockTest) Run(factory tests.TestContextFactory) (result error) {
	config, ok := tests.Forks[t.Data().Network]
	if !ok {
	...
	gblock, err := gspec.Commit(db, triedb) // TODO tady se naplni databaze podle PreAlloc (shodné s factory pro StateTest)
	if err != nil {
		return err
	}
	triedb.Close() // close the db to prevent memory leak

	if gblock.Hash() != t.Data().Genesis.Hash {
		return fmt.Errorf("genesis block hash doesn't match test: computed=%x, test=%x", gblock.Hash().Bytes()[:6], t.json.Genesis.Hash[:6])
	}
	if gblock.Root() != t.Data().Genesis.StateRoot {
