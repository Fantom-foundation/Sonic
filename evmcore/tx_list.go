...
func (l *txList) Add(tx *types.Transaction, priceBump uint64) (bool, *types.Transaction) {
	...
	l.txs.Put(tx)
	if cost := tx.Cost(); new(big.Int).SetBytes(l.costcap.Bytes()).Cmp(cost) < 0 {
		l.costcap = new(uint256.Int).SetBytes(cost.Bytes())
	}
...
func (l *txList) Filter(costLimit *uint256.Int, gasLimit uint64) (types.Transactions, types.Transactions) {
	...
	removed := l.txs.Filter(func(tx *types.Transaction) bool {
		return tx.Gas() > gasLimit || tx.Cost().Cmp(new(big.Int).SetBytes(costLimit.Bytes())) > 0
	})
