...
func TestFileHash_ReadWrite(t *testing.T) {
	...
	t.Run("Large file, pieceSize=10000", func(t *testing.T) {
		testFileHash_ReadWrite(t, bytes.Repeat([]byte(FILE_CONTENT), 20), ltypes.HexToHash("0xe3c3075749531525b472f4d6d70578e6d497d3e75b0727fea1ee10bdd1fcd490"), 10000)
	})
	t.Run("Large file, pieceSize=100", func(t *testing.T) {
		testFileHash_ReadWrite(t, bytes.Repeat([]byte(FILE_CONTENT), 20), ltypes.HexToHash("0xdc6d882fde82b2dd44884a97884d79be40e1d0f780a493dee0f7256d8261f7a5"), 100)
	})
	t.Run("Medium file, pieceSize=1", func(t *testing.T) {
		testFileHash_ReadWrite(t, bytes.Repeat([]byte(FILE_CONTENT), 1), ltypes.HexToHash("0x63a76929ee27decd5100d07a3cb626c05df1f1e927c5f27fa17c62459685ca6f"), 1)
	})
	t.Run("Medium file, pieceSize=2", func(t *testing.T) {
		testFileHash_ReadWrite(t, bytes.Repeat([]byte(FILE_CONTENT), 1), ltypes.HexToHash("0x2babd1049c449a60da62a1a0a3dc836e6a222dece07dcba1890a041d60ff29c7"), 2)
	})
	t.Run("Medium file, pieceSize=100", func(t *testing.T) {
		testFileHash_ReadWrite(t, bytes.Repeat([]byte(FILE_CONTENT), 1), ltypes.HexToHash("0x15c45aba675b7c49f5def32b4f24e827d478e5dfd712613fd6a5df69a2793b60"), 100)
	})
	t.Run("Tiny file, pieceSize=1", func(t *testing.T) {
		testFileHash_ReadWrite(t, []byte{0}, ltypes.HexToHash("0xbdda25ac486f2b8c0330a6fcace8f3d05d3e713b7920a39a1f60c0d8df024c0e"), 1)
	})
	t.Run("Tiny file, pieceSize=2", func(t *testing.T) {
		testFileHash_ReadWrite(t, []byte{0}, ltypes.HexToHash("0xf2b22424f7d1d01467d650f18ad49df8929fbefdeefe95e868d52eec6ea399e1"), 2)
	})
	t.Run("Empty file, pieceSize=1", func(t *testing.T) {
		testFileHash_ReadWrite(t, []byte{}, ltypes.HexToHash("0x9cbc73d18d70c94fe366e696035c4f2cffdbab7ea6d6c2c039ca185f9c9f2746"), 1)
	})
	t.Run("Empty file, pieceSize=2", func(t *testing.T) {
		testFileHash_ReadWrite(t, []byte{}, ltypes.HexToHash("0x163e7f66d58036ccb1d0b0058d8f46e7cd639816f570e5eb32853ea73634e4cd"), 2)
	})
}

func testFileHash_ReadWrite(t *testing.T, content []byte, expRoot ltypes.Hash, pieceSize uint64) {
	...
	// passing the wrong root hash to reader
	{
		file, err = os.OpenFile(filePath, os.O_RDONLY, 0600)
		require.NoError(err)
		maliciousReader := WrapReader(file, maxMemUsage, ltypes.HexToHash("0x00"))
		data := make([]byte, 1)
