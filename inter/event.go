...
type EventI interface {
	...
	PrevEpochHash() *ltypes.Hash
	Extra() []byte
	GasPowerLeft() GasPowerLeft
	GasPowerUsed() uint64

	HashToSign() ltypes.Hash
	...
	PayloadHash() ltypes.Hash
}

type EventLocator struct {
	BaseHash    ltypes.Hash
	NetForkID   uint16
...
func EmptyPayloadHash(version uint8) ltypes.Hash {
	if version == 1 {
		return emptyPayloadHash1
	} else {
		return ltypes.Hash(types.EmptyRootHash)
	}
}
...
type Event struct {
	...
	// cache
	_baseHash    *ltypes.Hash
	_locatorHash *ltypes.Hash
}
...
func (e *Event) HashToSign() ltypes.Hash {
	return *e._locatorHash
}

func asLocator(basehash ltypes.Hash, e EventI) EventLocator {
	return EventLocator{
		BaseHash:    basehash,
...
func (e *extEventData) PrevEpochHash() *ltypes.Hash { return e.prevEpochHash }

func (e *extEventData) Extra() []byte { return e.extra }

func (e *extEventData) PayloadHash() ltypes.Hash { return e.payloadHash }
...
func CalcTxHash(txs types.Transactions) ltypes.Hash {
	return ltypes.Hash(types.DeriveSha(txs, trie.NewStackTrie(nil)))
}

func CalcMisbehaviourProofsHash(mps []MisbehaviourProof) ltypes.Hash {
	hasher := sha256.New()
	_ = rlp.Encode(hasher, mps)
	return ltypes.BytesToHash(hasher.Sum(nil))
}

func CalcPayloadHash(e EventPayloadI) ltypes.Hash {
	if e.Version() == 1 {
		return ltypes.Of(ltypes.Of(CalcTxHash(e.Txs()).Bytes(), CalcMisbehaviourProofsHash(e.MisbehaviourProofs()).Bytes()).Bytes(), ltypes.Of(e.EpochVote().Hash().Bytes(), e.BlockVotes().Hash().Bytes()).Bytes())
	} else {
...
func (e *MutableEventPayload) SetPrevEpochHash(v *ltypes.Hash) { e.prevEpochHash = v }

func (e *MutableEventPayload) SetExtra(v []byte) { e.extra = v }

func (e *MutableEventPayload) SetPayloadHash(v ltypes.Hash) { e.payloadHash = v }
...
func (e *MutableEventPayload) SetEpochVote(v LlrEpochVote) {
	e.epochVote = v
	e.anyEpochVote = v.Epoch != 0 && v.Vote != ltypes.Zero
}

func calcEventID(h ltypes.Hash) (id [24]byte) {
	copy(id[:], h[:24])
	return id
}

func calcEventHashes(ser []byte, e EventI) (locator ltypes.Hash, base ltypes.Hash) {
	base = ltypes.Of(ser)
	if e.Version() < 1 {
		return base, base
	}
...
func (e *MutableEventPayload) calcHashes() (locator ltypes.Hash, base ltypes.Hash) {
...
func (e *MutableEventPayload) HashToSign() ltypes.Hash {
	h, _ := e.calcHashes()
	return h
}
...
func (e *MutableEventPayload) build(locatorHash ltypes.Hash, baseHash ltypes.Hash, size int) *EventPayload {
	return &EventPayload{
...
func (e *MutableEventPayload) immutable() *EventPayload {
	return e.build(ltypes.Hash{}, ltypes.Hash{}, 0)
}
...
func (l EventLocator) HashToSign() ltypes.Hash {
	return ltypes.Of(l.BaseHash.Bytes(), bigendian.Uint16ToBytes(l.NetForkID), l.Epoch.Bytes(), l.Seq.Bytes(), l.Lamport.Bytes(), l.Creator.Bytes(), l.PayloadHash.Bytes())
}

func (l EventLocator) ID() ltypes.EventHash {
	h := l.HashToSign()
	copy(h[0:4], l.Epoch.Bytes())
	copy(h[4:8], l.Lamport.Bytes())
	return ltypes.EventHash(h)
}
