...
func TestGetExecutionOrder_EntriesAreSorted(t *testing.T) {
	const size = 10
	tests := []struct {
		name string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			unsorted := make([]*scramblerEntry, len(entries))
			// make deep copy to make sure entries were sorted
			copy(unsorted, entries)
			entries = getExecutionOrder(entries, test.sort)
			for index, _ := range entries {
				// pass test at first order change
				if entries[index] != unsorted[index] {
					return
				}
			}

			t.Error("entries does not seem to be sorted")
		})
	}
}
