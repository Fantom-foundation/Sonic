...
func (s *Store) GetLiveStateDb(stateRoot hash.Hash) (state.StateDB, error) {
	...
	stateHash := stateDb.GetStateHash()
	if stateHash != common.Hash(stateRoot) {
		stateDb.Release()
		return nil, fmt.Errorf("unable to get Carmen live StateDB - unexpected state root (%x != %x)", stateHash, stateRoot)
	}
	return stateDb, nil
}
...
func (s *Store) GetRpcStateDb(blockNum *big.Int, stateRoot common.Hash) (state.StateDB, error) {
	...
	hash := stateDb.GetStateHash()
	if hash != stateRoot {
		stateDb.Release()
		return nil, fmt.Errorf("unable to get Carmen archive StateDB - unexpected state root (%x != %x)", hash, stateRoot)
	}
	return stateDb, nil
}
