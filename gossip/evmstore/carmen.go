...
func (c *carmenStateDB) GetBalance(addr common.Address) *big.Int {
	return c.tx.GetBalance(carmen.Address(addr))
}
...
func (c *carmenStateDB) AddBalance(addr common.Address, amount *big.Int) {
	c.tx.AddBalance(carmen.Address(addr), amount)
}

func (c *carmenStateDB) SubBalance(addr common.Address, amount *big.Int) {
	c.tx.SubBalance(carmen.Address(addr), amount)
}
...
func (c *carmenStateDB) Suicide(addr common.Address) bool {
	return c.tx.SelfDestruct(carmen.Address(addr))
}
...
func (s *carmenTxPoolStateDB) GetBalance(addr common.Address) *big.Int {
	var res *big.Int
	err := s.db.QueryHeadState(func(ctxt carmen.QueryContext) {
		res = ctxt.GetBalance(carmen.Address(addr))
	})
	if err != nil {
